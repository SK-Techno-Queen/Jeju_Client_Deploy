...
const KakaoMap = () => {
  ...
  const activeInfoWindowRef = useRef(null); // 열린 인포윈도우를 추적하기 위한 useRef
  ...
  useEffect(() => {
    ...
    // 이전 마커들을 지도에서 제거
    markerMap.current.forEach((marker, plateNo) => {
      const busExists = buses.find(bus => bus.plateNo === plateNo);
      if (!busExists) {
        marker.setMap(null); // 마커 제거
        markerMap.current.delete(plateNo); // markerMap에서 삭제
      }
    });
    
  }, [buses, map]); // 의존성 배열에서 markers는 제거
...
useEffect(() => {
  ...
  // Clean up markers on unmount or change
  return () => {
